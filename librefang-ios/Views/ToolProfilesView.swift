...
private struct ToolProfilesSupportCoverageDeck: View {
    let visibleProfileCount: Int
    let totalProfileCount: Int
    let selectedProfileName: String?
    let selectedToolCount: Int
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            MonitoringSnapshotCard(
                summary: summaryLine,
                detail: String(localized: "Use this deck to keep selected-profile support, visible catalog breadth, and scoped search readable before pivoting into fleet, runtime, or integration context."),
                verticalPadding: 4
            ) {
                FlowLayout(spacing: 8) {
                    if let selectedProfileName {
            ...
            MonitoringFactsRow {
                VStack(alignment: .leading, spacing: 3) {
                    Text(String(localized: "Support coverage"))
                        .font(.subheadline.weight(.medium))
                    Text(String(localized: "Keep selected-profile depth, broader catalog size, and dense bundled-tool support readable before leaving the profile directory."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(2)
                }
    ...
    private var summaryLine: String {
        if hasSelectedProfile {
            return String(localized: "Tool-profile support coverage is currently anchored by the selected profile and its bundled tools.")
        }
        if hasSearchScope {
            return String(localized: "Tool-profile support coverage is currently narrowed to the filtered catalog slice.")
        }
        if densestVisibleToolCount > 12 {
            return String(localized: "Tool-profile support coverage is currently anchored by dense bundled-tool catalogs.")
        }
        return String(localized: "Tool-profile support coverage is currently balanced across the visible catalog.")
    }
}
...
private struct ToolProfilesActionReadinessDeck: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            MonitoringSnapshotCard(
                summary: summaryLine,
                detail: String(localized: "Use this deck to check route breadth, selected-profile depth, and visible catalog coverage before leaving the compact tool-profile directory."),
                verticalPadding: 4
            ) {
                FlowLayout(spacing: 8) {
                    PresentationToneBadge(
            ...
            MonitoringFactsRow {
                VStack(alignment: .leading, spacing: 3) {
                    Text(String(localized: "Action readiness"))
                        .font(.subheadline.weight(.medium))
                    Text(String(localized: "Keep route breadth, selected-profile state, and visible catalog size readable before moving into fleet, runtime, or integration context."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(2)
                }
                ...
                if densestVisibleToolCount > 0 {
                    Label(
                        densestVisibleToolCount == 1 ? String(localized: "1 tool in top profile") : String(localized: "\(densestVisibleToolCount) tools in top profile"),
    ...
    private var summaryLine: String {
        if hasSelectedProfile {
            return String(localized: "Tool-profile action readiness is currently anchored by the selected profile and its bundled tools.")
        }
        if hasSearchScope {
            return String(localized: "Tool-profile action readiness is currently anchored by the filtered catalog slice and nearby route exits.")
        }
        if densestVisibleToolCount > 12 {
            return String(localized: "Tool-profile action readiness is currently anchored by dense bundled-tool catalogs.")
        }
        return String(localized: "Tool-profile action readiness is currently clear enough for compact route pivots and catalog review.")
    }
}
